...
leaflegend = L.LeafLegend = L.Class.extend(
  ...
  getLegendHTML: (map) ->
    ...
    legendObject.push("<span style=\"height:" + @options.cell_width +  "px;\">" + "<div style=\"width:" + (2*@options.cell_width) + "px; position: relative; height: 0; text-align: left; color: #{label1_color_option};" + "  -webkit-transform: translate(#{(col)*increment_in_em+1-increment_in_em}em,-6em); -moz-transform: translate(#{(col)*increment_in_em+1-increment_in_em}em,-6em); -o-transform: translate(#{(col)*increment_in_em+1-increment_in_em}em,-6em); -ms-transform: translate(#{(col)*increment_in_em+1-increment_in_em}em,-6em); -webkit-transform: translate(#{(col)*increment_in_em+4-increment_in_em}em,-4em); transform: translate(#{(col)*increment_in_em+4-increment_in_em}em,-4em);\">" + "<div style=\"-webkit-transform: rotate(-45deg); -moz-transform: rotate(-45deg);\">" + gdrow[col].y + "</div></div></span>") for col in [0...ysize]
